...
import (
	...
	"github.com/GoAsyncFunc/server-trojan/internal/pkg/dispatcher"
	"github.com/GoAsyncFunc/server-trojan/internal/pkg/service"
...
const (
	...
	LogLevelInfo   = "info"
	LogLevelError  = "error"
	DefaultDataDir = "/var/lib/server-trojan"
)
...
func (s *Server) loadCore(inboundConfig *core.InboundHandlerConfig, outboundConfig *core.OutboundHandlerConfig) (*core.Config, error) {
	...
	// Check strict equality might be failing if casing differs? Or defaults?
	// But clearer logic:
	if s.config.LogLevel == LogLevelDebug {
		logConfig.LogLevel = "debug"
		logConfig.AccessLog = "" // Empty string means stdout/stderr usually, or default file? Xray defaults to stdout if not set?
		// Actually Xray conf: "AccessLog": "/path/to/file" or "" for none/stdout?
		// documentation: "AccessLog": "" -> stdout. "none" -> disable.
		// If we want debug, we want stdout normally.
	} else {
